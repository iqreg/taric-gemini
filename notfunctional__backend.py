#!/usr/bin/env python3
"""
Bulk-Auswertung von Produktbildern gegen das bestehende TARIC-Backend (Variante A).

Funktion:
- Liest alle Bilder aus data/taric_bulk_input/
- Schickt EIN Bild pro Request an den bestehenden Endpunkt (z. B. POST /classify mit Multipart "file")
- Steuert die Request-Rate über --sleep-per-request, um Gemini-API-Limits einzuhalten
...
# In deinem Backend: @app.post("/classify")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Maximale Anzahl Bilder pro Batch (default: 30)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=7.0,
        help="Pause (Sekunden) nach JEDEM Request, um API-Rate-Limits einzuhalten (default: 7.0)",
    )
    ...
    logger.info("Sleep pro Request (Sek.): %.2f", args.sleep_per_request)
    ...
    for batch_idx, batch in enumerate(chunked(all_files, args.batch_size), start=1):
        ...
        for image_path in batch:
            ...
            # Pause nach jedem Request (Rate-Limit)
            if args.sleep_per_request > 0:
                time.sleep(args.sleep_per_request)
